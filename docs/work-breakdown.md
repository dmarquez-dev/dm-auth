...
|----|------|-------------|--------|--------------|
| 1.1 | Create .NET solution and project structure | Create `DM-Auth.sln` with four src projects (DMAuth.Domain, DMAuth.Application, DMAuth.Infrastructure, DMAuth.Web) and two test projects (DMAuth.Tests.Unit, DMAuth.Tests.Integration). Add `Directory.Build.props` (net9.0, nullable enable, implicit usings). Configure project references: Web → Application → Domain; Infrastructure → Application → Domain. | TODO | — |
